const { connection, getTableName } = require('./routesCore.js')

// Route 3: /closest-restaurants/:type
const closestRestaurants = async function(req, res) {
    ...
    SELECT name, latitude, longitude, zipcode
    FROM LiberalResults
    WHERE (
      (69 * DEGREES(ACOS(COS(RADIANS(SELECT initial_lat FROM InitialLocation))  * 
      COS(RADIANS(latitude))  * COS(RADIANS(longitude) - 
      RADIANS(SELECT initial_lng FROM InitialLocation)) + SIN(RADIANS(SELECT initial_lat FROM InitialLocation)) * 
